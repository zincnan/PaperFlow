...
def get_value(config: dict[str, Any], path: tuple[str, ...], default: Any = None) -> Any:
    current: Any = config
...
def normalize_venue(venue_obj: Any) -> str:
    ...
    if isinstance(venue_obj, list):
        return "; ".join(str(item).strip() for item in venue_obj if str(item).strip())
...
def normalize_record(hit: dict[str, Any], query: str, rank: int) -> dict[str, Any]:
    info = hit.get("info", {})
...
def search_dblp_with_retry(
    ...
    raise RuntimeError(f"Query failed after {retries} attempts: {query}") from last_error


def write_output(output_path: Path, payload: dict[str, Any], jsonp_callback: str) -> None:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Search DBLP publications from YAML config and save JSONP output"
    )
    parser.add_argument(
        "-c",
        "--config",
    ...
    base_url = str(get_value(config, ("network", "base_url"), "https://dblp.org")).strip()
    ...
    results_per_query = int(get_value(config, ("search", "results_per_query"), 50))
    year_filter = str(get_value(config, ("search", "year"), "")).strip()

    output_parent_dir = Path(
        str(get_value(config, ("output", "parent_dir"), "./dblp_results"))
    ).expanduser()
    ...
    save_raw_results = bool(get_value(config, ("output", "save_raw_results"), True))
    ...
    log_info(f"Output directory: {output_parent_dir}")
    ...
    for query in queries:
        try:
            raw_result = search_dblp_with_retry(
                api_url=api_url,
                query=query,
                results_per_query=results_per_query,
                proxies=proxies,
    ...
    extension = "jsonp" if jsonp_callback else "json"
    output_path = output_parent_dir / f"dblp_search_{timestamp}.{extension}"
